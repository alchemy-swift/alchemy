...
/// Represents a dynamic parameter inside the URL. Parameter placeholders should
/// be prefaced with a colon (`:`) in the route string. Something like
/// `:user_id` in the path `/v1/users/:user_id`.
public struct PathParameter: Equatable {
    /// An error encountered while decoding a path parameter value string to a
    /// specific type such as `UUID` or `Int`.
    ...
    /// is an invalid `UUID`.
    ///
    /// - Throws: a `PathParameter.DecodingError` if the value string is not
