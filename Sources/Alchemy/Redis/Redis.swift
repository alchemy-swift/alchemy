...
public final class RedisTransaction: RedisClient {
    struct Message {
        let cmd: String
        let args: [RESPValue]
    }
    ...
    public var eventLoop: EventLoop { Services.eventLoop }
    
    public func logging(to logger: Logger) -> RedisClient { self }
    
    public func send(command: String, with arguments: [RESPValue]) -> EventLoopFuture<RESPValue> {
        fatalError()//self.messages.append(Message(cmd: command, args: arguments))
    }
...
struct TransactionBuilder {
    static func buildBlock(_ futures: EventLoopFuture<Void>...) -> [EventLoopFuture<Void>] {
        return futures
    }
}

extension RedisClient {
    func transaction(@TransactionBuilder _ txn: (RedisClient) -> [EventLoopFuture<Void>]) -> EventLoopFuture<Void> {
        return self.command("MULTI")
            .flatMap { _ in
                fatalError()
            }
            .flatMap { self.command("EXEC") }
            .voided()
    }
    
    func test() {
        self.transaction { (client: RedisClient) in
            client.set(RedisKey("key1"), to: 1)
            client.set(RedisKey("key2"), to: 2)
            client.set(RedisKey("key3"), to: 3)
            client.set(RedisKey("key4"), to: 4)
        }
    }
}
