...
extension Endpoint {
    ...
    ///              `Session.default`.
    ///   - jsonEncoder: The `JSONEncoder` to use when encoding the `Request`. Defaults to
    ///                  `JSONEncoder()`.
    ///   - jsonDecoder: The `JSONDecoder` to use when decoding the `Response`. Defaults to
    ///                  `JSONDecoder()`.
    ///   - completion: A completion that will be called when the request is complete. Contains the
    ///                 raw `AFDataResponse<Data>` as well as a `Result` containing either the
