...
final class ClientResponseTests: XCTestCase {
    ...
    func testBody() {
        ...
        XCTAssertEqual(ClientResponse(response: .with(body: body)).content, Content(buffer: body, contentType: nil))
        XCTAssertEqual(ClientResponse(response: .with(headers: ["content-type": "application/json"], body: body)).content, Content(buffer: body, contentType: .json))
