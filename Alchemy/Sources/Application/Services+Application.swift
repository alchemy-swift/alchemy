...
extension Container {
    ...
    @Service(.singleton) var _application: Application? = nil
    @Service(.singleton) public var lifecycle = Lifecycle(app: application)
    @Service(.singleton) public var env: Environment = .createDefault()
    @Service(.singleton) public var threadPool: NIOThreadPool = .singleton
    @Service(.singleton) public var eventLoopGroup: EventLoopGroup = MultiThreadedEventLoopGroup(numberOfThreads: isTest ? 1 : System.coreCount)

    @Service public var eventLoop: EventLoop = {
